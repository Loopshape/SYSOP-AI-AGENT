#!/usr/bin/env bash
# ai.sh - AI Autonomic Synthesis Platform v30.1 (Parser-Safe Edition)
# An agent with a fixed Triumvirate logic pipeline that streams its verbose thinking in real-time.
...
<style>:root{--bg:#0d1117;--text:#c9d1d9;--accent:#58a6ff;--secondary:#8b949e;--border:#3036d;--input-bg:#161b22;--success:#3fb950;--error:#f85149;}
...
http.createServer((req, res) => {
    ...
    if (req.url === '/api/command' && req.method === 'POST') {
        ...
        req.on('end', () => {
            try {
                // FIX: Replaced JS destructuring with explicit assignment to be Bash-parser-safe.
...
# ---------------- AUTONOMOUS WORKFLOW (Triumvirate Logic with Verbose Streaming) ----------------
run_agi_workflow() {
    local user_prompt="$*"
    ...
    for ((i=1; i<=MAX_AGENT_LOOPS; i++)); do
        ...
        local messenger_prompt="You are the Messenger. Analyze the context and provide a structured summary of the goal and current state."
        ...
        for model in "${PLANNER_MODELS[@]}"; do
            ...
            (
                ...
                local planner_prompt="You are a strategic Planner. Based on the Messenger's analysis, create a concise, step-by-step plan. Propose ONE specific tool to use next."
...
show_help() {
    cat << EOF
${GREEN}AI Autonomic Synthesis Platform v30.1 (Live Streaming Edition)${NC}
